const projects = [
  { number: '01', title: 'Product Lab', type: 'Product exploration · Interactive prototypes', description: 'A collection of ten working product concepts across research, media, collaboration, public life, knowledge and wellbeing—each with its own focused interactive experience.', tags: ['React', 'TypeScript', 'Product thinking'], live: 'https://more-projects-lab.vercel.app/', code: 'https://github.com/tristanlgb/more-projects', tone: 'green' },
  { number: '02', title: 'TaskFlow', type: 'Productivity · Task management', description: 'A polished task management workspace for organizing projects with categories, status tracking, due dates, image attachments, search, and persistent browser storage.', tags: ['React', 'TypeScript', 'Product design'], live: 'https://taskmanagerreact-pi.vercel.app/', code: 'https://github.com/tristanlgb/taskmanager', tone: 'blue' },
  { number: '03', title: 'Social Observatory', type: 'Data visualization · Social research', description: 'A responsible, accessible dashboard that compares socioeconomic samples from 2016 and 2024 while making methodological limits and privacy considerations explicit.', tags: ['Data storytelling', 'Chart.js', 'Accessible UI'], live: 'https://app-graficos-clase-social.vercel.app/', code: 'https://github.com/tristanlgb/AppGraficosClaseSocial', tone: 'green' },
  { number: '04', title: 'Interactive Pokédex', type: 'Interactive systems · 3D experience', description: 'An immersive Pokédex that turns API data into an exploratory interface with interactive 3D elements and visual representations of processes and interruptions.', tags: ['Interactive 3D', 'Process visualization', 'PokéAPI'], live: 'https://pokedex-pokeapi-ruddy.vercel.app/', code: '', tone: 'blue' },
...
function Projects() {
  return <section id="projects" className="projects-section"><div className="shell">
    ...
    <div className="project-list">{projects.map((project) => <article className={`project-row ${project.tone}`} key={project.title}>
      ...
      <div className="project-links">{project.live ? <a href={project.live} target="_blank" rel="noreferrer">Live project ↗</a> : null}{project.code ? <a href={project.code} target="_blank" rel="noreferrer">View source ↗</a> : null}</div>
    </article>)}</div>
  </div></section>;
}
